...
PAGES.overview = () => {
  ...
  <div class="grid k4" style="margin-bottom:14px">
    ${stat('Calls handled','1,142','','+12.4%','up','vs. previous 24 h',spark(D.calls24),true)}
    ${stat('Talk minutes','2,918','','+8.1%','up','avg 2 m 33 s per call',spark(D.minutes24,'var(--violet)'))}
    ${stat('Time to first byte','704','ms','−3.2%','up','p50 across all agents',spark(D.ttfb.map(v=>1500-v),'var(--brand)'))}
    ${stat('Spend','$89.14','','+5.6%','down','LLM + TTS + STT + carrier',spark(D.minutes24.map(v=>v*0.7),'var(--warn)'))}
  </div>
  ...
  <div class="split" style="margin-bottom:14px">
    <div class="card">
      ...
      ${area(D.calls24,{labels:hrs, unit:' calls'})}
      <div class="legend"><span><i style="background:var(--brand)"></i>Calls started</span>
        <span><i style="background:var(--line)"></i>Peak 74 calls at 17:00</span></div>
    </div>
    <div class="card">
  ...
  <div class="split" style="margin-bottom:14px">
    ...
    <div class="card">
      ...
      ${D.services.map(([n,u,st,cpu,mem])=>`
        <div class="mrow">
          <span class="nm">${n} <span class="t-sub mono">${u}</span></span>
          <span class="vl">${cpu}% · ${mem} MB</span>
  ...
  <div class="split">
    ...
    <div class="card">
      ...
      <div class="rows">${D.events.map(([t,k,w])=>`
...
PAGES.agents = () => `
  <div class="page-head"><div><h2>Voice Agents</h2><p>Each agent is a prompt plus a speech pipeline: speech-to-text, a language model, and a voice.</p></div>
    <span class="spacer"></span><span class="pill warn nodot">Community Edition — 2 of 2 voice agents used</span>
    <button class="btn">${ico('plus',16)} New agent</button></div>
  <div class="agents">${D.agents.filter(a=>a.t==='voice').map(a=>`
    <article class="agent">
    ...
    <article class="agent" style="border-style:dashed;display:grid;place-items:center;min-height:190px">
      <div style="text-align:center;padding:20px">
        <div class="face" style="margin:0 auto 10px">${ico('plus',20)}</div>
        <b style="font-family:var(--f-display)">Add a third agent</b>
        <div class="t-sub" style="margin-top:4px;max-width:30ch">Community Edition caps you at two. Upgrade the licence to lift it.</div>
      </div></article>
  </div>
...
PAGES.trunks = () => `
  ...
  ${table([['Trunk'],['Carrier'],['Direction'],['Address'],['State']],
    D.trunks.map(t=>`<tr><td class="mono t-main">${t[0]}</td><td>${t[1]}</td>
...
PAGES.rules = () => `
  ...
  ${table([['#',1],['Rule'],['Number'],['Sends to'],['When']],
    D.rules.map((r,i)=>`<tr><td class="num">${i+1}</td><td class="t-main">${r[0]}</td>
...
PAGES.numbers = () => `
  ...
  ${table([['Number'],['Region'],['Routes to'],['Direction']],
    D.numbers.map(n=>`<tr><td class="mono t-main">${n[0]}</td><td>${n[1]}</td>
...
PAGES.settings = () => {
  ...
  <div class="split">
    ...
    <div class="card">${head('This server')}
      ...
      <div class="mrow"><span class="nm">Licence</span><span class="vl mono">Community Edition</span></div>
      <div class="note" style="margin-top:6px">${ico('bell',16)}
        <span><b>Community Edition</b> is free for non-commercial use and caps you at two voice and two chatbot agents.</span></div>
    </div></div>`;
};
