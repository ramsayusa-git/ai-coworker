...
export function registerTools(s: McpServer, api: Api, z: Z) {
  ...
  // ---- Order changes & approvals ----
  s.tool('freshrice_order_changes', 'Audit trail of modifications and discounts on an order.', { orderId: z.string() }, wrap(async (a) => api(`/orders/${a.orderId}/modifications`)));
  s.tool('freshrice_order_modify', 'WRITE. Change an order: ITEMS [{skuId,qty}], SLOT (deliveryDate YYYY-MM-DD, slotId), ADDRESS (addressId) or NOTE. Reason is shown to the customer.', { orderId: z.string(), type: z.enum(['ITEMS', 'SLOT', 'ADDRESS', 'NOTE']), reason: z.string(), items: z.array(z.object({ skuId: z.string(), qty: z.number().int() })).optional(), deliveryDate: z.string().optional(), slotId: z.string().nullable().optional(), addressId: z.string().optional(), note: z.string().optional(), collectOnDelivery: z.boolean().optional() }, wrap(async (a) => { const { orderId, ...b } = a; return api(`/admin/orders/${orderId}/modify`, { body: b }); }));
  s.tool('freshrice_order_discount', 'WRITE. Goodwill discount on an order (rupees or pct). Applies immediately if within the API key role limit, otherwise queues for approval.', { orderId: z.string(), amountRupees: z.number().optional(), pct: z.number().optional(), reason: z.string(), issueId: z.string().optional() }, wrap(async (a) => api(`/admin/orders/${a.orderId}/discount`, { body: { amountPaise: a.amountRupees ? Math.round(a.amountRupees * 100) : undefined, pct: a.pct, reason: a.reason, issueId: a.issueId } })));
  s.tool('freshrice_approvals', 'Discount requests waiting for approval (or APPROVED/REJECTED history).', { status: z.enum(['PENDING_APPROVAL', 'APPROVED', 'REJECTED', 'APPLIED']).default('PENDING_APPROVAL') }, wrap(async (a) => (await api(`/admin/approvals?status=${a.status}`)).map((m: any) => ({ id: m.id, order: m.order.orderNo, customer: m.order.user.name, amount: rupees(m.amountPaise), reason: m.reason, requestedBy: `${m.requester?.name} (${m.requestedRole})`, createdAt: m.createdAt, status: m.status, decision: m.decisionNote, canApprove: m.canApprove }))));
  s.tool('freshrice_approval_decide', 'WRITE. Approve or reject a pending discount (note required to reject).', { approvalId: z.string(), approve: z.boolean(), note: z.string().optional() }, wrap(async (a) => api(`/admin/approvals/${a.approvalId}/${a.approve ? 'approve' : 'reject'}`, { body: { note: a.note } })));

  // ---- HR & Fleet ----
  s.tool('freshrice_hr_today', 'Who is clocked in / late / absent / on leave today, plus pending leave requests count.', {}, wrap(async () => api('/admin/hr/today')));
  s.tool('freshrice_hr_attendance', 'Monthly attendance summary per staff member (present/half/absent/leave/late/hours/payable days).', { month: z.string().describe('YYYY-MM').optional(), userId: z.string().optional() }, wrap(async (a) => { const r = await api(`/admin/hr/attendance?month=${a.month || new Date().toISOString().slice(0, 7)}${a.userId ? `&userId=${a.userId}` : ''}`); return r.rows.map((x: any) => ({ userId: x.user.id, name: x.user.name, role: x.user.role, ...x.summary })); }));
  s.tool('freshrice_hr_leave_queue', 'Leave / WFH / regularisation requests by status.', { status: z.enum(['PENDING', 'APPROVED', 'REJECTED', 'CANCELLED']).default('PENDING') }, wrap(async (a) => api(`/hr/leave-queue?status=${a.status}`)));
  s.tool('freshrice_hr_leave_decide', 'WRITE. Approve or reject a leave request (note required to reject).', { requestId: z.string(), approve: z.boolean(), note: z.string().optional() }, wrap(async (a) => api(`/hr/leave/${a.requestId}/${a.approve ? 'approve' : 'reject'}`, { body: { note: a.note } })));
  s.tool('freshrice_hr_payroll', 'Payroll sheet for a month (payable days, LOP, OT, net payable per employee).', { month: z.string().optional() }, wrap(async (a) => api(`/admin/hr/payroll?month=${a.month || new Date().toISOString().slice(0, 7)}`)));
  s.tool('freshrice_fleet_dashboard', 'Fleet overview: vehicle counts, alerts (docs/service due), month cost, cost per drop per vehicle.', { month: z.string().optional() }, wrap(async (a) => api(`/admin/fleet/dashboard${a.month ? `?month=${a.month}` : ''}`)));
  s.tool('freshrice_fleet_vehicles', 'Vehicles with rider, odometer, status, doc alerts, last pre-trip check.', {}, wrap(async () => api('/admin/fleet/vehicles')));
  s.tool('freshrice_fleet_vendors', 'Vehicle hire vendors with balance owed.', {}, wrap(async () => api('/admin/fleet/vendors')));
  s.tool('freshrice_fleet_log_expense', 'WRITE. Log a vehicle expense (FUEL/MAINTENANCE/REPAIR/TOLL/FINE/OTHER).', { vehicleId: z.string(), type: z.enum(['FUEL', 'MAINTENANCE', 'REPAIR', 'TOLL', 'FINE', 'OTHER']), amountRupees: z.number(), odometerKm: z.number().optional(), litres: z.number().optional(), vendorName: z.string().optional(), description: z.string().optional(), nextDueKm: z.number().optional() }, wrap(async (a) => { const { vehicleId, amountRupees, ...b } = a; return api(`/admin/fleet/vehicles/${vehicleId}/logs`, { body: { ...b, amountPaise: Math.round(amountRupees * 100) } }); }));
