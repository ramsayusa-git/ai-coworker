...
export default function Team() {
  ...
  return <div><Toast />
    ...
    <div className="card overflow-auto"><table className="tbl"><thead><tr><th>Name</th><th>Phone</th><th>Role</th><th>Warehouse</th><th>Leads</th><th>Status</th><th></th></tr></thead><tbody>
      {data?.map((s: any) => <tr key={s.id} className={!s.active ? 'opacity-50' : ''}>
        <td>{s.name}</td><td>{s.phone}</td><td><span className="badge bg-gray-100">{s.role}</span></td>
