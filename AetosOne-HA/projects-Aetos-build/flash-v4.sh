...
set -e
DEV=/dev/sdb
echo "== flashing aetos-one-rpi4-build-v4.img.xz -> $DEV =="
sudo -n podman run --rm --privileged -v /dev:/dev \
  ...
  docker.io/library/alpine:latest sh -c '
    ...
    echo "writing (~5.4GB)..."
    xzcat /img/aetos-one-rpi4-build-v4.img.xz | pv -pterb | dd of=/dev/sdb bs=4M conv=fsync
    sync
