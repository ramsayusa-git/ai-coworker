...
# Make image
# 6G working size (pre-warm removed in v9.8.8); resize2fs -M
# below shrinks the fs back to its true minimum so the FINAL image only grows by
# the real unpacked content.
rm -f "${data_img}"
...
# AETOS ONE: seed HA config only (v7.3 = plain HTTP on 8123, no SSL/cert baked)
mkdir -p "${data_dir}/supervisor/homeassistant"
tar xf /build/buildroot-external/package/hassio/aetos-seed.tar -C "${data_dir}/supervisor/homeassistant"

# AETOS ONE v12: PRE-REGISTER the curated add-on repositories.
# Supervisor reads store.json at startup, so the 5 Aetos repos are present
# immediately instead of waiting for aetos-addons-firstboot (observed taking
# ~2 hours on a slow card). The firstboot registrar remains as a safety net.
cp /build/buildroot-external/package/hassio/aetos-store.json "${data_dir}/supervisor/store.json"
