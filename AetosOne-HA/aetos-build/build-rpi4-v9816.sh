#!/bin/bash
# Aetos One v9.8.16 rpi4 - Core 2026.8.0 locked. FIX: removed server_port:80
...
LOG=$BASE/v9816-build.log
: > "$LOG"
exec > >(tee -a "$LOG") 2>&1
echo "===== v9.8.16 RPI4 BUILD start $(date '+%F %H:%M') ====="
...
[ -n "$RPIIMG" ] && cp "$RPIIMG" "$DELIV/aetos-one-rpi4-build-v9.8.16.img.xz" \
  && echo "  RPI4 OK -> aetos-one-rpi4-build-v9.8.16.img.xz $(du -h "$DELIV/aetos-one-rpi4-build-v9.8.16.img.xz"|cut -f1)" \
...
echo ""; echo "===== v9.8.16 RPI4 DONE $(date '+%F %H:%M') ====="
ls -lh "$DELIV"/aetos-one-rpi4-build-v9.8.16.img.xz
