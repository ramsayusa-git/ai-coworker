#!/bin/bash
# Stop the in-flight v9.8.8 build cleanly so it can be restarted with the
# updated seed (sidebar-hide brand.js).
pkill -TERM -f build-rpi4-v988.sh 2>/dev/null
...
echo "STOPPED+CLEANED $(date '+%H:%M:%S')"
