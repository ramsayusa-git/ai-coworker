#!/bin/bash
# Aetos One v9.8.10 rpi4 - Core 2026.8.0 locked; HA serves on port 80 natively
...
LOG=$BASE/v9810-build.log
: > "$LOG"
exec > >(tee -a "$LOG") 2>&1
echo "===== v9.8.10 RPI4 BUILD start $(date '+%F %H:%M') ====="
...
echo "  port80 disabled:  $([ -e $OV/usr/lib/systemd/system/multi-user.target.wants/aetos-port80.service ] && echo NO || echo YES)"
...
[ -n "$RPIIMG" ] && cp "$RPIIMG" "$DELIV/aetos-one-rpi4-build-v9.8.10.img.xz" \
  && echo "  RPI4 OK -> aetos-one-rpi4-build-v9.8.10.img.xz $(du -h "$DELIV/aetos-one-rpi4-build-v9.8.10.img.xz"|cut -f1)" \
  || { echo "  RPI4 FAILED"; exit 1; }
echo ""; echo "===== v9.8.10 RPI4 DONE $(date '+%F %H:%M') ====="
ls -lh "$DELIV"/aetos-one-rpi4-build-v9.8.10.img.xz
