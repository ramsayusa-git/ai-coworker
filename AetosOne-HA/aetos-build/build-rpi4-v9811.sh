#!/bin/bash
# Aetos One v9.8.11 rpi4 - Core 2026.8.0 locked; HA serves on port 80 natively
...
LOG=$BASE/v9811-build.log
: > "$LOG"
exec > >(tee -a "$LOG") 2>&1
echo "===== v9.8.11 RPI4 BUILD start $(date '+%F %H:%M') ====="
...
[ -n "$RPIIMG" ] && cp "$RPIIMG" "$DELIV/aetos-one-rpi4-build-v9.8.11.img.xz" \
  && echo "  RPI4 OK -> aetos-one-rpi4-build-v9.8.11.img.xz $(du -h "$DELIV/aetos-one-rpi4-build-v9.8.11.img.xz"|cut -f1)" \
  || { echo "  RPI4 FAILED"; exit 1; }
echo ""; echo "===== v9.8.11 RPI4 DONE $(date '+%F %H:%M') ====="
ls -lh "$DELIV"/aetos-one-rpi4-build-v9.8.11.img.xz
