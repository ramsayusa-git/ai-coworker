...
export async function conversationsRoutes(app: FastifyInstance) {
  ...
  app.post("/orgs/:orgId/conversations/:id/messages", { preHandler: requireCapability("send_messages") }, async (req, reply) => {
    const { orgId, id } = req.params as { orgId: string; id: string };
    const { body, templateId, interactive, variables } = req.body as {
      body?: string; templateId?: string; interactive?: any; variables?: string[];
    };
    if (!body?.trim() && !templateId) return reply.status(400).send({ error: "body required" });

    // One shared outbound path (text / interactive / template) — also meters the
    // conversation against the wallet and emits the message.sent webhook event.
    const result = await sendOnConversation({
      orgId, conversationId: id, body, templateId, interactive, variables,
    });
