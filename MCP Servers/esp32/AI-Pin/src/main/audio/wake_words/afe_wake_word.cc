...
// AI-Pin: digital gain applied to mic samples before feeding the AFE/WakeNet.
// The XIAO S3 Sense PDM mic (MSM261) produces a lower amplitude signal than the
// std-I2S mics most xiaozhi boards use; server-side ASR copes fine but WakeNet
// is amplitude sensitive. Gain is applied with saturation. Set to 1 to disable.
#define AFE_FEED_GAIN 4
...
bool AfeWakeWord::Initialize(AudioCodec* codec, srmodel_list_t* models_list) {
    ...
    // AI-Pin: disable the WebRTC VAD stage in the wake-word AFE. Nothing here
    // consumes vad_state, and removing it eliminates any chance of the VAD
    // gating/starving WakeNet on this quiet PDM mic.
...
void AfeWakeWord::Feed(const std::vector<int16_t>& data) {
    ...
    // AI-Pin: apply digital gain with saturation and track peak levels.
    static int feed_chunk_counter = 0;
    static int32_t peak_in = 0, peak_out = 0;
    size_t base = input_buffer_.size();
    input_buffer_.resize(base + data.size());
    for (size_t i = 0; i < data.size(); i++) {
        int32_t s = data[i];
        int32_t a = s < 0 ? -s : s;
        if (a > peak_in) peak_in = a;
        int32_t amplified = s * AFE_FEED_GAIN;
        if (amplified > 32767) amplified = 32767;
        else if (amplified < -32768) amplified = -32768;
        input_buffer_[base + i] = (int16_t)amplified;
        int32_t aa = amplified < 0 ? -amplified : amplified;
        if (aa > peak_out) peak_out = aa;
    }
    if (++feed_chunk_counter >= 500) {
        ESP_LOGI(TAG, "Feed levels: peak_in=%ld peak_out=%ld gain=%d", (long)peak_in, (long)peak_out, AFE_FEED_GAIN);
        feed_chunk_counter = 0;
        peak_in = 0;
        peak_out = 0;
    }
