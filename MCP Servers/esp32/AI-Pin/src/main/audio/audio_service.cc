...
void AudioService::PushTaskToEncodeQueue(AudioTaskType type, std::vector<int16_t>&& pcm) {
    ...
    // AI-Pin fix: never block forever here. If the encode queue stays full
    // (e.g. the websocket died while the audio processor kept running, so the
    // send queue was never drained), the old infinite wait wedged the
    // audio-processor task inside its output callback: AFE fetch stopped,
    // "Ringbuffer of AFE(FEED) is full" spammed forever, and speech playback
    // broke up until reboot. Wait briefly, then drop the oldest task instead.
    if (!audio_queue_cv_.wait_for(lock, std::chrono::milliseconds(200),
        ...
        static uint32_t dropped_tasks = 0;
        if (!audio_encode_queue_.empty()) {
            audio_encode_queue_.pop_front();
        }
        if ((++dropped_tasks % 50) == 1) {
            ESP_LOGW(TAG, "Encode queue stalled, dropped %lu oldest tasks so far", (unsigned long)dropped_tasks);
        }
    }
