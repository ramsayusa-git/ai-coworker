...
NoAudioCodecSimplexPdm::NoAudioCodecSimplexPdm(int input_sample_rate, int output_sample_rate, gpio_num_t spk_bclk, gpio_num_t spk_ws, gpio_num_t spk_dout, i2s_std_slot_mask_t spk_slot_mask, gpio_num_t mic_sck, gpio_num_t mic_din) {
    ...
    // Raise the PDM downsample rate from the default DSR_8S to DSR_16S.
    // At a 16kHz sample rate the default clocks the PDM mic at only
    // 1.024MHz - the exact bottom edge of typical PDM mic specs (e.g. the
    // XIAO ESP32S3 Sense's MSM261 is rated 1.0-3.25MHz), which degrades
    // audio quality enough that server ASR mishears consonants
    // ("Jarvis" -> "Jaise") and on-device wake models never trigger.
    // DSR_16S doubles the mic clock to a healthy 2.048MHz.
