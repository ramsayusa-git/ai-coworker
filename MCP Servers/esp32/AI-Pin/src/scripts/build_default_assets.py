...
def main():
    ...
    if custom_wake_word_config and multinet_model_paths:
        ...
        # CONFIG_CUSTOM_WAKE_WORD may contain MULTIPLE alternate spellings
        # separated by ';' (useful for brand words the g2p mispronounces) -
        # each becomes its own command, all mapping to the same display text.
        wake_word_variants = [w.strip() for w in custom_wake_word_config['wake_word'].split(';') if w.strip()]
        multinet_model_info = {
            "language": language,
