...
try {
    ...
    # Always sync the 'main' branch (master was merged into main 2026-08-28)
    $branch = "main"

    # 1) PULL first: bring down all folders/files added on GitHub or other machines
    git pull origin $branch --no-edit --allow-unrelated-histories --quiet 2>$null
    ...
    if ($statusOutput) {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            git push $remoteUrl HEAD:$branch --quiet 2>$null
