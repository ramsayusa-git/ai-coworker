"""Central engine: guided-step state machine, sample processing, recording,
and fan-out to WebSocket clients / MQTT.
...
STEP_TEXT = {
    ...
    "live":       ("Live", "Streaming. Recording is available."),
...
class Engine:
    def __init__(self, data_dir: str, mains_hz: int = 50, loop: asyncio.AbstractEventLoop | None = None):
        ...
        self.control_source_ble = None      # set by main: BleSource for start/stop buttons
        self._history: list = []            # last ~10 s filtered samples for late joiners
    ...
    def handle_packet(self, pkt, source: str):
        ...
        if not st.contact:
            st.contact = True
        if st.step in ("contact", "checking", "connecting", "mqtt_wait", "scanning", "wake", "idle", "error"):
        ...
        for s in pkt.samples:
            ...
            if self._rec_writer:
                self._rec_writer.writerow([self._rec_index, f"{self._rec_index / NOMINAL_FS:.4f}", pkt.seq, s, f"{f:.1f}"])
                self._rec_index += 1
        if beats:
        ...
        elif st.step == "live" and st.packets % 66 == 0:
    ...
    def stop_recording(self) -> str:
        ...
        if self._rec_file:
            ...
            with open(os.path.join(self.sessions_dir, name.replace(".csv", ".json")), "w") as f:
                json.dump(meta, f)
            self._log(f"Recording stopped: {name} ({meta['seconds']} s)")
        ...
        for fn in sorted(os.listdir(self.sessions_dir), reverse=True):
            ...
            if os.path.exists(mp):
                try:
                    ...
                    pass
            out.append({"file": fn, "bytes": os.path.getsize(p), "mtime": int(os.path.getmtime(p)), **meta})
        return out

    def delete_session(self, fn: str) -> bool:
    ...
    async def broadcaster(self):
        """Flush pending samples to WS clients ~20x/s; also the stream watchdog."""
        tick = 0
        while True:
            ...
            if tick % 20 == 0:
                st = self.status
                if st.step in ("signal", "live") and self._last_packet_t and time.time() - self._last_packet_t > 2.5:
                    if st.connected:
                        st.contact = False
                    ...
                    self._broadcast_status()      # keep counters/uptime fresh; 30 s MQTT heartbeat
            ...
            if self.clients:
                self._send_all(json.dumps({"type": "ecg", "fs": NOMINAL_FS, "beats": beats,
                                           "hr": self.status.hr, "s": batch}))

    def history_message(self) -> str:
        return json.dumps({"type": "ecg", "fs": NOMINAL_FS, "beats": 0, "hr": self.status.hr,
                           "s": self._history[-int(NOMINAL_FS * 6):], "history": True})
