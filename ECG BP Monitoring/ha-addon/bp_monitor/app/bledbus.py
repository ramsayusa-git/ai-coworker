...
# THE critical detail for this cuff. Device1.Connect() connects every
# *supported* profile, and for a device BlueZ has cached as dual-mode it tries
# the BR/EDR profiles first — of which this cuff has none, so the call fails
# with BREDR.ProfileUnavailable no matter how many times it is retried.
#
# Scanning with Transport="le" makes BlueZ record the device as LE-only, and
# Connect() then performs a plain LE connection. (This is also why the original
# probe script worked: bleak's scanner had already set an LE discovery filter,
# so the cached record was LE. An unfiltered scan re-caches it as dual-mode and
# the connect starts failing again — so the filter is not optional here.)
LE_FILTER = {"Transport": Variant("s", "le"),
...
class DbusBle:
    ...
    async def set_le_filter(self):
        """Restrict discovery to LE — see LE_FILTER above; this is what makes
        the subsequent Connect() an LE connect instead of a doomed BR/EDR one."""
        try:
            await self._call(self.adapter_path, ADAPTER_IFACE,
                             "SetDiscoveryFilter", "a{sv}", [LE_FILTER])
        except Exception as e:
            log.warning("SetDiscoveryFilter(le) failed: %s", e)
    ...
    async def remove_device(self, path: str | None = None):
        """Drop BlueZ's cached record so the device can be re-learned under the
        LE discovery filter."""
        path = path or self.device_path
        if not path:
            return
        try:
            await self._call(self.adapter_path, ADAPTER_IFACE, "RemoveDevice",
                             "o", [path], timeout=10.0)
            log.info("removed cached device record %s", path.rsplit("/", 1)[-1])
        except Exception as e:
            log.debug("RemoveDevice: %s", e)

    async def _is_stale_bredr_record(self) -> bool:
        """True if the cached record was learned by a non-LE scan. Such a record
        makes Connect() attempt BR/EDR profiles and fail; it has to be dropped
        and re-learned with the LE filter on."""
        ...
        if props.get("Connected"):
            return False
        # "Class" is only published for records carrying BR/EDR class-of-device
    ...
    async def discover(self, timeout: float, on_tick=None) -> bool:
        ...
        if self.device_path:
            # A record left behind by an unfiltered scan will fail to connect,
            # so throw it away and let the LE-filtered scan below re-learn it.
            if await self._is_stale_bredr_record():
                log.info("cached record is BR/EDR-flagged — re-learning over LE")
    ...
    async def _connect_le_explicit(self) -> bool:
        """Adapter1.ConnectDevice — asks BlueZ for an LE connection explicitly.
        Only available when bluetoothd runs with --experimental, so treat a
        failure as "not available" rather than as a real error."""
        try:
            await self._call(self.adapter_path, ADAPTER_IFACE, "ConnectDevice",
                             "a{sv}", [{"Address": Variant("s", self.address),
                                        "AddressType": Variant("s", "public")}],
                             timeout=30.0)
            log.info("ConnectDevice(le) succeeded")
            return True
        except Exception as e:
            log.debug("ConnectDevice unavailable/failed: %s", e)
            return False

    async def connect(self, attempts: int = 3, on_attempt=None) -> None:
        """Connect over LE, with a recovery ladder that re-learns the device
        under the LE discovery filter rather than retrying a doomed call."""
        if not self.device_path:
        ...
        for i in range(attempts):
            ...
            try:
                ...
                if i == attempts - 1:
                    break
                keep_going = await self._recover(e)
                if self.connected:
                    return          # _recover got us connected (LE ConnectDevice)
                if not keep_going:
                    break
        raise BleError(str(last) or "connect failed")

    async def _recover(self, err: Exception) -> bool:
        """Cleanup between connect attempts. Returns False if there is no point
        trying again. ProfileUnavailable means BlueZ is treating the device as
        BR/EDR-capable, so the only useful move is to forget the record and
        re-learn it with the LE filter on — retrying Connect() as-is cannot
        succeed."""
        msg = str(err).lower()
        try:
        ...
        if "profileunavailable" in msg or "br-connection" in msg:
            if await self._connect_le_explicit():
                try:
                    await self._wait_services()
                    self.connected = True
                    return False        # already connected — stop the ladder
                except Exception:
                    pass
            await self.remove_device()
            self.device_path = None
            if not await self.discover(30.0):
                log.warning("device did not re-appear after LE re-discovery")
                return False
            return True

        await self.remove_device()
        self.device_path = None
        return await self.discover(25.0)
